...
function MessageBlock({ message, isSameActor, isDarkMode = false }: MessageBlockProps) {
  ...
  return (
    <div
      ...
      {!isSameActor && (
        <div
          className="flex size-8 shrink-0 items-center justify-center rounded-full text-lg"
      ...
      <div className="min-w-0 flex-1">
        {!isSameActor && (
          <div className={`mb-1 text-sm font-semibold ${isDarkMode ? 'text-white' : 'text-white'}`}>
            {actor.name}
          </div>
        )}

        <div className="space-y-0.5">
          <div className={`whitespace-pre-wrap break-words text-sm ${isDarkMode ? 'text-white' : 'text-white'}`}>
            {isProgress ? (
          ...
          {!isProgress && (
            <div className={`text-right text-xs ${isDarkMode ? 'text-white' : 'text-white'}`}>
