...
- key <string>: to send a sequence of keypresses (e.g., "h i enter"). The syntax for this is that each key is space-separated. So if you want to type "hello", you would use "key h e l l o". To write a string, type each letter space-separated. To press a modifier key, use the modifier name (e.g., "key ctrl+t"). To press a special key, use the special key name (e.g., "key tab").
